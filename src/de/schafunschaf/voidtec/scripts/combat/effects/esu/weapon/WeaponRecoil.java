...
public class WeaponRecoil extends BaseStatMod {
    public WeaponRecoil(float baseValue) {
        super(baseValue);
    }

    @Override
    public void apply(MutableShipStatsAPI stats, String id, Random random, UpgradeQuality quality) {
        stats.getMaxRecoilMult().modifyPercent(id, 1f - generateModValue(random, ));
        stats.getRecoilPerShotMult().modifyPercent(id, 1f - generateModValue(random, ));
        stats.getRecoilDecayMult().modifyPercent(id, 1f - generateModValue(random, ));
    }
